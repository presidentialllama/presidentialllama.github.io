...
function appendTableRows() {
    ...
    for (let i = 0; i < clubs.length; i++) {
        ...
        cell5.innerHTML =
            "<button class='btn btn-success cmn_noPadding cmn_fullHeight' onclick='displayClubDistanceEntryForm(" +
            i +
            ");'>&nbsp;&nbsp;+&nbsp;&nbsp;</button>";
...
function appendTapEntryButtons() {
    ...
    for (let i = avgDistPlusSome; i > avgDistMinusSome; i -= 5) {
        ...
        if (i == Math.round(clubs[clubRow][3])) {
            btn.innerHTML =
                "<button class='cmn_noPadding cmn_fullHeight cmn_tapEntry' onclick='updateStats(" + i + ");'><b>" + i + "</b></button> ";
...
function updateStats(shotDistance = 0) {
    ...
    if (parseInt(shotDistance) > 0) {
        ...
        // update max
        if (clubs[clubRow][5] == 0 || shotDistance > clubs[clubRow][5]) clubs[clubRow][5] = shotDistance;
        // save updated stats in local storage
        str = JSON.stringify(clubs);
        localStorage.setItem("clubs", str);
        // return to list screen
