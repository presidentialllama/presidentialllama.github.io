...
function loadContent() {
    // if api call exists and is older than 24 hours, call again
    if (localStorage.getItem("apicall") === null || dayjs().diff(dayjs(localStorage.getItem("apicall")["Date"]), "hours") >= 24) {
        ...
        xhttp.onreadystatechange = function () {
            if (this.readyState == 4 && this.status == 200) {
                ...
                localStorage.setItem("apicall", covidJson);
            } // end if
        ...
        // api call is less than 24h old, use localstorage

        covidJson = localStorage.getItem("apicall");
...
function displayGraph() {
    ...
    totalDeathsOver50000 = [];

    for (let c of covidJsObj.Countries) {
        if (c.TotalDeaths > 50000) {
            totalDeathsOver50000.push({
                Slug: c.Slug,
                TotalConfirmed: c.TotalConfirmed,
                TotalDeaths: c.TotalDeaths,
                TotalConfirmedPer100000: (parseInt(c.TotalConfirmed) / parseInt(populations[c.Slug])) * 100_000,
            });
        }
    }
    totalDeathsOver50000 = _.orderBy(totalDeathsOver50000, "TotalConfirmedPer100000", "desc");
    ...
    chartData.data.datasets[2].label = "cases per 100,000";
    chartData.data.labels = totalDeathsOver50000.map((x) => x.Slug);
    chartData.data.datasets[0].data = totalDeathsOver50000.map((x) => x.TotalConfirmed);
    chartData.data.datasets[1].data = totalDeathsOver50000.map((x) => x.TotalDeaths);
    chartData.data.datasets[2].data = totalDeathsOver50000.map((x) => x.TotalConfirmedPer100000);
    chartData.options.title.text = "Covid 19 Hotspots for " + dayjs().format("MM/DD/YYYY"); // add today's date to char title
    myChart = new Chart(ctx, chartData);

    // step2
    var newArray = [];
    for (let i = 0; i < covidJsObj.Countries.length; i++) {
        let totalConfirmed = covidJsObj.Countries[i].TotalConfirmed;
        let population = populations[covidJsObj.Countries[i].Slug];
        newArray.push({
            Slug: '"' + covidJsObj.Countries[i].Slug + '"',
            TotalConfirmed: totalConfirmed,
            TotalDeaths: covidJsObj.Countries[i].TotalDeaths,
            Population: population,
            TotalConfirmedPer100000: (parseInt(totalConfirmed) / parseInt(population)) * 100_000,
        });
    }
}
